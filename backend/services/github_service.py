...
def _sync_create_pr(patches: list[dict]) -> dict:
    """Blocking PyGithub calls — run via executor only."""
...
async def create_patch_pr(patches: list[dict]) -> dict:
    """Create a GitHub PR with LLM-generated patches — async wrapper."""
    try:
...
async def health_check() -> bool:
    """Try to connect to GitHub and verify repo access."""
    try:
        loop = asyncio.get_event_loop()
